...
var LicerepoReport = __webpack_require__(/*! ./LicerepoReport */ "./bin/LicerepoReport.js");
var Licerepo = function () {
  ...
  return _createClass(Licerepo, [{
    ...
    key: "report",
    value: function () {
      var _report = _asyncToGenerator(_regenerator().m(function _callee4(packageType, packageName) {
        var version,
          _args4 = arguments;
        return _regenerator().w(function (_context4) {
          while (1) switch (_context4.n) {
            case 0:
              version = _args4.length > 2 && _args4[2] !== undefined ? _args4[2] : 'latest';
              _context4.n = 1;
              return new LicerepoReport().report(packageType, packageName, version);
            case 1:
              return _context4.a(2, _context4.v);
          }
        }, _callee4);
      }));
      function report(_x4, _x5) {
...
var _require = __webpack_require__(/*! lodash */ "./node_modules/lodash/lodash.js"),
  last = _require.last;
var LicerepoReport = function () {
  function LicerepoReport() {
    _classCallCheck(this, LicerepoReport);
    _defineProperty(this, "LIBRARY_DATA", null);
  }
  return _createClass(LicerepoReport, [{
    key: "report",
    value: (function () {
      var _report = _asyncToGenerator(_regenerator().m(function _callee(packageType, packageName) {
        var version,
          ptype,
          library,
          dep,
        ...
        return _regenerator().w(function (_context) {
          while (1) switch (_context.n) {
            case 0:
              version = _args.length > 2 && _args[2] !== undefined ? _args[2] : 'latest';
              ptype = this._validatePackageType(packageType);
              if (ptype.valid) {
                _context.n = 1;
                break;
              }
              throw new Error(ptype.reason);
            case 1:
              library = null, dep = null;
              _t = ptype.type;
              _context.n = _t === 'npm' ? 2 : _t === 'composer' ? 4 : _t === 'pub' ? 6 : 8;
              break;
            case 2:
              dep = new LicerepoNpm();
              _context.n = 3;
              return dep.fetch(packageName, version);
            case 3:
              library = _context.v;
              return _context.a(3, 8);
            case 4:
              dep = new LicerepoComposer();
              _context.n = 5;
              return dep.fetch(packageName, version);
            case 5:
              library = _context.v;
            ...
            case 9:
              this.LIBRARY_DATA = library;
              _context.n = 10;
              return this._getLicense(this.LIBRARY_DATA.license);
            case 10:
              return _context.a(2, _context.v);
          }
        }, _callee, this);
    ...
    key: "_getLicense",
    value: (function () {
      var _getLicense2 = _asyncToGenerator(_regenerator().m(function _callee2(license) {
        var report, flag;
        return _regenerator().w(function (_context2) {
          while (1) switch (_context2.n) {
            case 0:
              report = {
                ...
                flag: true,
                reason: 'Absense or, non-standard license. Must be manually checked.',
                status: 'unknown',
                lastChecked: new Date().toISOString().slice(0, 19).replace('T', ' '),
                metadata: this.LIBRARY_DATA
              };
              license = license.toLowerCase().trim();
              report.license = license;
              if (!(typeof report.license !== 'string')) {
                _context2.n = 1;
                break;
              }
              report.license = typeof report.license === 'string' ? report.license : null;
              return _context2.a(2, report);
            case 1:
              report.reason = null;
              flag = this._flagLicense(report.license);
              report.flag = flag.isFlagged;
              report.reason = flag.reason;
              report.status = flag.status;
              return _context2.a(2, this.generateEuComplianceReport(flag));
          }
        }, _callee2, this);
      }));
      function _getLicense(_x3) {
        return _getLicense2.apply(this, arguments);
      }
      return _getLicense;
    }())
  }, {
    key: "generateEuComplianceReport",
    value: function generateEuComplianceReport() {
      var _analysis$flag, _meta$dependencies, _meta$dependencies2;
      var licenseAnalysis = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : null;
      var analysis = licenseAnalysis || {
        flag: true,
        reason: 'No license analysis available',
        status: 'unknown',
        license: null,
        lastChecked: new Date().toISOString().slice(0, 19).replace('T', ' ')
      };
      var meta = this.LIBRARY_DATA || {};
      var riskLevel = 'Low';
      var messages = [];
      if (analysis.flag) {
        if (analysis.status === 'restrictive') {
          riskLevel = 'High';
          messages.push("Main package license is restrictive (".concat(analysis.license, "): ").concat(analysis.reason));
        } else if (analysis.status === 'caution' || analysis.status === 'unknown') {
          riskLevel = 'Medium';
          messages.push("Main package license requires review (".concat(analysis.status, "): ").concat(analysis.reason || 'Unknown license'));
        }
      }
      if (meta.deprecated === true) {
        riskLevel = riskLevel === 'Low' ? 'Medium' : riskLevel;
        messages.push('Package is marked as deprecated');
      }
      if (meta.hasVulnerabilities === true || (meta.vulnerabilitiesCount || 0) > 0) {
        riskLevel = riskLevel === 'Low' ? 'Medium' : riskLevel;
        messages.push("Known vulnerabilities present (".concat(meta.vulnerabilitiesCount || 'some', ")"));
      }
      if (messages.length === 0) {
        messages.push('No major license, vulnerability or maintenance red flags detected');
      }
      var report = {
        "package": {
        ...
        mainLicense: {
          declared: analysis.license || meta.license || 'Not declared',
          status: analysis.status || 'unknown',
          flagged: (_analysis$flag = analysis.flag) !== null && _analysis$flag !== void 0 ? _analysis$flag : true,
          reason: analysis.reason || null,
          commercialUseSafe: !analysis.flag,
          copyleftRisk: analysis.status === 'restrictive' ? 'High' : 'Low/None'
        },
        dependencies: {
          total: ((_meta$dependencies = meta.dependencies) === null || _meta$dependencies === void 0 ? void 0 : _meta$dependencies.total) || 0,
        ...
        risk: {
          ...
          hasVulnerabilities: !!meta.hasVulnerabilities,
          vulnerabilitiesCount: meta.vulnerabilitiesCount || 0,
          recommendation: this.assessDependencyRisk(((_meta$dependencies2 = meta.dependencies) === null || _meta$dependencies2 === void 0 ? void 0 : _meta$dependencies2.total) || 0)
        },
    ...
    key: "assessDependencyRisk",
    value: function assessDependencyRisk(totalDependencies) {
      if (!Number.isInteger(totalDependencies) || totalDependencies < 0) {
        return {
          score: 0,
          level: 'invalid',
          category: 'unknown',
          recommendation: 'Invalid input: totalDependencies must be a non-negative integer'
        };
      }
      var score = 0;
      var level = 'low';
      ...
      if (totalDependencies <= 50) {
        category = 'small';
        score = Math.min(2, totalDependencies / 25);
        level = 'low';
        recommendation = totalDependencies === 0 ? 'No dependencies — excellent. Minimal attack surface and maintenance burden.' : "Very low risk (".concat(totalDependencies, " deps). This is a healthy, small integration. Continue monitoring for updates.");
      } else if (totalDependencies <= 250) {
        category = 'medium';
        score = 3 + Math.min(4, (totalDependencies - 50) / 50);
        level = totalDependencies <= 150 ? 'low-moderate' : 'moderate';
        recommendation = "Moderate dependency count (".concat(totalDependencies, "). ") + "Still maintainable for most teams, but consider auditing the most critical ones " + "(security, maintenance status, license compatibility).";
      } else if (totalDependencies <= 800) {
        category = 'large';
        score = 7 + Math.min(2, (totalDependencies - 250) / 275);
        level = 'high';
        recommendation = "High dependency count (".concat(totalDependencies, "). ") + "Significant supply-chain risk, maintenance burden and potential for vulnerabilities. " + "Strongly consider:\n" + "\u2022 Dependency pruning / deduplication\n" + "\u2022 Vendoring critical components\n" + "\u2022 Automated SBOM + vulnerability scanning\n" + "\u2022 Regular dependency health reviews";
      } else {
        category = 'large';
        score = 9 + Math.min(1, (totalDependencies - 800) / 1200);
        level = 'very-high';
        recommendation = "Very high dependency count (".concat(totalDependencies, "). ") + "This level is commonly considered \"dependency hell\". " + "Major refactoring is strongly recommended:\n" + "\u2022 Break monolith into smaller modules\n" + "\u2022 Replace heavy abstractions with lighter alternatives\n" + "\u2022 Vendor or fork critical dependencies\n" + "\u2022 Invest in automated dependency governance";
      }
      return {
    ...
    key: "_flagLicense",
    value: function _flagLicense(license) {
      if (!license || typeof license !== 'string') {
        return {
          isFlagged: true,
          reason: 'Missing or invalid license information',
          status: 'unknown'
        };
      }
      var normalized = license.toLowerCase().trim().replace(/\s+/g, '-').replace(/^(the\s+|mit\s+license|apache\s+2|bsd\s+)/i, '');
      var commercialSafe = new Set(['mit', 'mit-0', 'isc', 'unlicense', 'bsd-2-clause', 'bsd-3-clause', 'bsd-3-clause-clear', 'apache-2.0', 'apache-1.1', 'apache-1.0', 'zlib', 'boost', 'cc0-1.0', 'wtfpl', 'mpl-2.0', 'mpl-1.1', 'epl-2.0', 'epl-1.0', 'bsd-0-clause', '0bsd', 'public-domain', 'beerware', 'mit-no-advertisement']);
      var restrictivePatterns = [{
        pattern: /gpl-?([23]|v?3\.?0?)/i,
        reason: 'Strong copyleft (GPL) – requires derivative works to be open source under GPL'
      }, {
        pattern: /agpl/i,
        reason: 'Network copyleft (AGPL) – even remote use triggers source disclosure'
      }, {
        pattern: /lgpl/i,
        reason: 'Lesser GPL – still has copyleft obligations for modifications'
      }, {
        pattern: /copyleft/i,
        reason: 'Copyleft license – may require source disclosure'
      }, {
        pattern: /cc-?by-?(nc|nd|sa)/i,
        reason: 'Creative Commons with NC (non-commercial) or ND (no derivatives) – not suitable for software/commercial use'
      }, {
        pattern: /non.?commercial/i,
        reason: 'Explicitly prohibits commercial use'
      }, {
        pattern: /sspl/i,
        reason: 'Server Side Public License – not OSI-approved, strong copyleft for hosted services'
      }, {
        pattern: /cdl/i,
        reason: 'Common Development and Distribution License – has attribution & patent clauses that may cause issues'
      }, {
        pattern: /cern-?ohl/i,
        reason: 'CERN Open Hardware License – may have unusual conditions'
      }, {
        pattern: /eupl/i,
        reason: 'European Union Public License – has copyleft and some jurisdiction-specific clauses'
      }, {
        pattern: /^proprietary$/i,
        reason: 'Proprietary / closed-source license – cannot be redistributed or modified'
      }, {
        pattern: /all rights reserved/i,
        reason: 'No license granted – default copyright, commercial use not permitted'
      }, {
        pattern: /no.?license/i,
        reason: 'No license specified – default copyright applies, cannot be used commercially'
      }];
      if (commercialSafe.has(normalized)) {
        return {
          isFlagged: false,
          reason: null,
          status: 'safe'
      ...
      for (var _i = 0, _restrictivePatterns = restrictivePatterns; _i < _restrictivePatterns.length; _i++) {
        var _restrictivePatterns$ = _restrictivePatterns[_i],
          pattern = _restrictivePatterns$.pattern,
          reason = _restrictivePatterns$.reason;
        if (pattern.test(normalized) || pattern.test(license)) {
          return {
            isFlagged: true,
            reason: reason,
            status: 'restrictive'
      ...
      if (/fair|source/i.test(normalized) || /busl/i.test(normalized) || /elastic/i.test(normalized) || /server.?side/i.test(normalized) || normalized.includes('commons-clause')) {
        return {
          isFlagged: true,
          reason: 'Non-standard or source-available license with commercial/hosting restrictions.',
          status: 'caution'
        };
      }
      return {
        isFlagged: true,
        reason: "License '".concat(license, "' is unknown or not clearly permissive for commercial use."),
        status: 'unknown'
      };
    }
  }, {
    key: "_validatePackageType",
    value: function _validatePackageType(packageType) {
      if (typeof packageType !== 'string') {
        return {
          valid: false,
          reason: 'Package type must be a string.',
          type: null
        };
      }
...
/******/ 	__webpack_require__("./bin/LicerepoYaml.js");
/******/ 	__webpack_require__("./bin/LicerepoReport.js");
